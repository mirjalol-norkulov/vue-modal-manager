# modal-lifecycle Specification

## Purpose
How a modal moves between open and closed, and what happens to its props on the way: merging
props supplied at open, restoring the registration snapshot at close, and settling the promise
`openAsync()` returns — which must always settle and must never reject.
