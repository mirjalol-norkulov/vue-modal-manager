# modal-typing Specification

## Purpose
The public type surface of `useModal()`: how the props type is inferred from the component
supplied rather than written by hand, what a prop outside that surface produces, how a
component whose props cannot be resolved stays usable, and how the result types of
`openAsync()` and `close(result?)` relate.
