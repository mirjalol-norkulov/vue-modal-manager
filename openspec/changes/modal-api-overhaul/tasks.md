...
- [x] 8.4 Type the per-modal configuration options so a partial explicit pair is rejected. A plain union of
...
- [x] 8.5 Relate the `openAsync()` resolution type to the `close(result?)` argument type, widened with
...
- [x] 8.6 Confirm `pnpm type-check` passes and `dist/index.d.ts` still emits as a single flat declaration.
...
- [x] 9.1 `modal-rendering`: modal rendered once with resolved open prop; props bound; wrapped content still
      renders, in the same position relative to the modals as before the rewrite
- [x] 9.2 `modal-rendering`: `update:show` becomes `onUpdate:show`; an already-prefixed name is unchanged
- [x] 9.3 `modal-rendering`: precedence — explicit pair beats per-modal preset beats application default
- [x] 9.4 `modal-rendering`: two UI kits coexist in one application
- [x] 9.5 `modal-rendering`: unresolvable configuration errors naming the modal id, and a `preset` absent
      from the preset table raises that same error rather than a `TypeError`
- [x] 9.6 `modal-rendering`: plugin installs with no options when modals self-configure
- [x] 9.7 `modal-rendering`: default and named slots render; slot functions forwarded unwrapped; no-slots case
- [x] 9.8 `modal-lifecycle`: open merges props; snapshot uncorrupted; reset restores it, including across a
      second open/close cycle after an in-place mutation — the case that fails if restore aliases the snapshot
- [x] 9.9 `modal-lifecycle`: handle-close and self-close reset identically; `resetPropsOnClose: false` retains
- [x] 9.10 `modal-lifecycle`: `openAsync()` resolves with the `close(result)` value, and `undefined` for every
      other close path
- [x] 9.11 `modal-lifecycle`: `closeAll()`, unmount, and reopen all settle pending promises with `undefined`;
      a plain `open()` during a pending promise leaves it pending until the eventual close
- [x] 9.12 `modal-lifecycle`: no settlement path rejects; dismissal resolves
- [x] 9.13 `modal-lifecycle`: `openAsync()` during a server render resolves immediately and leaves the modal
      closed
- [x] 9.14 `modal-typing`: props inferred with no explicit type argument; explicit argument still accepted
- [x] 9.15 `modal-typing`: unknown prop rejected at registration and at open; wrong value type rejected
- [x] 9.16 `modal-typing`: props partial at every call site, including a component with required props
- [x] 9.17 `modal-typing`: `openAsync()` resolution includes `undefined`; `open()` is not a promise
- [x] 9.18 `modal-typing`: valid preset accepted, unknown preset rejected, half an explicit pair rejected
- [x] 9.19 `modal-typing`: a component whose props type cannot be resolved stays usable, and an `as any` props
      object type-checks for both a resolvable and an unresolvable component
- [x] 9.20 `modal-typing`: a `style` prop still type-checks, since `$props` carries the standard component
      attributes, while `key` and `ref` are rejected
- [x] 9.21 `modal-lifecycle`: `onOpen` fires once for `openAsync()`, and does not fire when the registry entry
      no longer exists

## 10. Documentation and changelog

- [x] 10.1 Document `slots` in `docs/api/composables.md`, including the slot-function shape
- [x] 10.2 Document `resetPropsOnClose`, and state that reset is shallow — nested objects are shared with the
      snapshot and are not restored
- [x] 10.3 Document `closeAllModals` on the handle alongside `useModalManager().closeAll`
- [x] 10.4 Document `openAsync()` and `close(result?)`, stating in the first sentence that the promise resolves
      when the modal **closes**, not when it opens, since the name does not convey this
- [x] 10.5 Document the settlement table: result, dismissal, close-all, unmount, server render, reopen
- [x] 10.6 Document per-modal `preset` / `openPropName` / `openEventName` and the precedence order, with a
      two-UI-kit example
- [x] 10.7 Correct the `UseModalReturnType` and `UseModalOptions` signatures in the API reference, which are
      currently out of date
- [x] 10.8 Complete `docs/migration/0-1-0.md`, created in `ssr-safe-store`, with the breaks owned by this
      change: `props` now type-checked (the headline), `slots` now rendering, and top-level props now
      resetting on close
- [x] 10.9 Give the props-typing section a worked before/after, and document the `props: {...} as any` escape
      hatch that unblocks a build while the real prop errors are fixed one at a time
- [x] 10.10 Add a summary table at the top of the guide listing every break with a "does this affect me?"
      column, so a reader can triage without reading the whole page
- [x] 10.11 Verify the migration guide resolves from both the sidebar and the nav in `pnpm docs:dev`, and that
      every code sample in it type-checks against the shipped `0.1.0` types
- [x] 10.12 Create `CHANGELOG.md` at the repo root in Keep a Changelog format, newest version first
- [x] 10.13 Write the `0.1.0` entry covering both this change and `ssr-safe-store`, grouped into Added /
...
- [x] 10.14 State that versions before `0.1.0` are not backfilled, pointing readers at the annotated tags
      `v0.0.2` through `v0.0.11`
- [x] 10.15 Verify `CHANGELOG.md` actually ships: `package.json` declares `files: ["dist"]`, and npm's
...
- [x] 10.16 Add a short "Upgrading to 0.1.0" section to `README.md` linking the migration guide. The npm
      package page renders the README and not the changelog, so this is the only upgrade signal an npm
      visitor sees
- [x] 10.17 Add a changelog step to `.agents/skills/release/SKILL.md`, before the version bump, so the entry
      is written as part of every release rather than only this one
- [x] 10.18 Run `pnpm skills:sync` to mirror the release skill into `.claude/skills/`, then confirm
      `pnpm skills:check` reports no drift
- [x] 10.19 Update `AGENTS.md`, which every coding agent reads first and which this change falsifies in three
...
- [x] 10.20 Fix the package name in the `<ModalProvider>` import sample in `docs/getting-started.md`, which
...
- [x] 11.1 Update `src/App.vue` so its currently-inert `default` slot is a live assertion that slots render
- [x] 11.2 Add a second modal from a different UI kit, or a custom component with its own `openPropName`, to
      exercise per-modal configuration
- [x] 11.3 Add a confirm-dialog example exercising `openAsync()` with `close(result)` wired through props

## 12. Verification

- [x] 12.1 `pnpm lint` and `pnpm format` clean
- [x] 12.2 `pnpm type-check` passes
- [x] 12.3 `pnpm test:unit run` fully green, and the type-test script separately green — type tests are a
      distinct vitest mode and will not run as part of `test:unit`
- [x] 12.4 `pnpm build` emits all four files and `exports` still resolves for both ESM and CJS consumers
...
Found reviewing the implementation against the specs above, after section 12 was green.
Two of these added requirements to the spec deltas, so they are recorded here rather than
folded silently into the sections they amend.

- [x] 13.1 `props` was `markRaw`ed on the way into the registry, which stopped the reactive
      registry unwrapping refs on the library's behalf — a `ref` in `props` reached the
      modal as a `Ref` and rendered `[object Object]`. `unwrapProps` in `ModalProvider.ts`
      binds each top-level value through `unref`, which also tracks it, so a ref value is
      now the supported way to keep one prop live. `ModalComponentProps<T>` widened to
      `MaybeRef` per entry to match — strictly wider, so it rejects nothing it used to
      accept. New `modal-rendering` requirement, two scenarios, two tests
- [x] 13.2 Registration copies `options.props`, so a modal no longer follows later
      mutations of the object the caller passed. That is deliberate — it is what makes the
      reset snapshot a snapshot and keeps `options` unwritten — but it changes runtime
      behaviour under an unchanged signature, so it belongs in the migration guide and the
      changelog beside `slots` and prop reset. New `modal-lifecycle` requirement, two
      scenarios, two tests
- [x] 13.3 `openAsync()` could still reject: a promise *adopts* a thenable rather than
      resolving with it, so `close(rejectingPromise)` rejected the returned promise. That
      breaks the never-rejects requirement in exactly the way it exists to prevent. The
      promise now carries a synchronously attached `catch` resolving `undefined`, and
      `settleModalResult` warns in development, so the swallowed error is not invisible.
      New `modal-lifecycle` scenario, one test
- [x] 13.4 Consolidate the dev-only warning guard into `warnInDevelopment` in `helpers.ts`
      now that two call sites need it, and move the narrow `process` declaration with it
- [x] 13.5 Documentation accuracy: `[preset table](/getting-started)` pointed at a page that
      never mentions presets; `components.md` implied an unresolvable modal fails alone when
      the throw actually aborts the whole provider render; the props-bound-after-open-state
      ordering was undocumented, and is reachable now that `props` is type-checked against a
      component whose open prop is a real prop name; and the confirm pattern silently
      resolves `undefined` if a dialog emits its own close event before the result callback
- [x] 13.6 Re-run section 12 in full after all of the above
